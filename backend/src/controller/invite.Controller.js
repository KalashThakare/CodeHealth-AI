...
export const listTeamInvites = async (req, res) => {
  try {
    ...
    const sortable = new Set(["createdAt", "updatedAt", "expiresAt", "email", "role"]);
    ...
    return res.status(200).json({ page, pageSize, total: count, items });
  } catch (error) {
...
export const listMyInvites = async (req, res) => {
  try {
    ...
    console.error("listReceivedInvites error:", error);
...
export const deleteTeamInvite = async (req, res) => {
  try {
    ...
    if (!teamId || !inviteId) {
      return res.status(400).json({ error: "teamId and inviteId are required" });
    }
    ...
    if (!allowed.includes((membership.role || "").toLowerCase())) {
      return res.status(403).json({ error: "Forbidden" });
    }

    const invite = await TeamInvite.findOne({ where: { id: inviteId, teamId } });
